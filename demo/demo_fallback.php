...
<body>
    <ol>
        ...
        $jstfa = new TwoFactorAuth('MyApp', 6, 30, 'sha1', $qrcodeprovider);

        $optimizeForRequests = false;
        ...
        echo '<li>Next create a QR code and let the user scan it:<br>' . $jstfa->getQRCodeHTML('My label', $secret);
        if ($optimizeForRequests) {
            echo '<noscript>'. $tfa->getQRCodeHTML('My label', $secret).'</noscript>';
        } else {
            // bandwidth optimized version
            echo '<noscript><img src="image.php?code='.$secret.'"></noscript>';
        }
        echo '<br>...or display the secret to the user for manual entry: ' . chunk_split($secret, 4, ' ');
